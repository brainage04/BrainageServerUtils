package com.github.brainage04.brainageserverutils.datagen;
...
import com.github.brainage04.brainageserverutils.util.StringUtils;
...
public class EnglishLangProvider extends FabricLanguageProvider {
    ...
    private final String autoConfigPrefix = "text.autoconfig.%s.option".formatted(BrainageServerUtils.MOD_ID);
    ...
    private void addAutomaticTranslations(String[] keys, String packageName, TranslationBuilder translationBuilder) {
        for (String key : keys) {
            translationBuilder.add("%s.%s.%s".formatted(packageName, BrainageServerUtils.MOD_ID, key), StringUtils.pascalCaseToHumanReadable(key));
        }
    }
    ...
    public void generateTranslations(RegistryWrapper.WrapperLookup registryLookup, TranslationBuilder translationBuilder) {
        ...
        translationBuilder.add(
                "key.category.%s".formatted(BrainageServerUtils.MOD_ID),
                BrainageServerUtils.MOD_NAME
        );
