...
public final class BrainageServerUtilsGameTest {
    ...
    public static void registerTestFunctions(RegisterEvent event) {
        event.register(BuiltInRegistries.TEST_FUNCTION.key(), Identifier.fromNamespaceAndPath(BrainageServerUtils.MOD_ID, "custom_rules_and_command_are_registered"), () -> BrainageServerUtilsGameTest::customRulesAndCommandAreRegistered);
        event.register(BuiltInRegistries.TEST_FUNCTION.key(), Identifier.fromNamespaceAndPath(BrainageServerUtils.MOD_ID, "item_and_consumption_rules_change_runtime_behavior"), () -> BrainageServerUtilsGameTest::itemAndConsumptionRulesChangeRuntimeBehavior);
    }

    private static void customRulesAndCommandAreRegistered(GameTestHelper context) {
        ...
        assertTrue(level.getServer().getCommands().getDispatcher().getRoot().getChild("brainagegamerule") != null, "Expected the compatibility gamerule command to be registered");
        context.succeed();
    }

    private static void itemAndConsumptionRulesChangeRuntimeBehavior(GameTestHelper context) {
        ServerLevel level = context.getLevel();
        MinecraftServer server = level.getServer();
        GameRules rules = level.getGameRules();
        Zombie user = EntityTypes.ZOMBIE.create(level, EntitySpawnReason.COMMAND);
        if (user == null) throw new AssertionError("Expected to create a zombie");
        ItemStack apples = new ItemStack(Items.APPLE, 3);
        Consumable consumable = apples.get(DataComponents.CONSUMABLE);
        if (consumable == null) throw new AssertionError("Expected apples to have a consumable component");
        try {
